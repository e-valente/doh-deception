...
class ZooAttack(EvasionAttack):
    ...
    def generate(self, x: np.ndarray, y: Optional[np.ndarray] = None, **kwargs) -> np.ndarray:
        ...
        if self.estimator.clip_values is not None:
            ...
            for i in self.feature_indices:
                x_adv[:, i] = np.clip(x_adv[:, i], clip_min[i], clip_max[i])
    ...
    def _generate_bss(
        ...
        feature_mask = np.zeros_like(x_orig)
        feature_mask[:, self.feature_indices] = 1
        ...
        for iter_ in range(self.max_iter):
            ...
            for i, (dist, pred) in enumerate(zip(l2dist, np.argmax(preds, axis=1))):
                if dist < best_dist[i] and self._compare(pred, labels_batch[i]):
                    best_dist[i] = dist
                    best_attack[i][self.feature_indices] = x_adv[i][self.feature_indices]
                    best_label[i] = pred
    ...
    def _optimizer(self, x: np.ndarray, targets: np.ndarray, c_batch: np.ndarray) -> np.ndarray:
        ...
        if self.feature_indices is not None:
            ...
            indices = np.arange(coord_batch.shape[-1])

        if self.use_importance and np.unique(self._sample_prob).size != 1:
            chosen_indices = (
                np.random.choice(indices, self.nb_parallel * self._current_noise.shape[0],
                                 replace=False, p=self._sample_prob.flatten()[indices],
                                 )
            )
        ...
        for i in range(self.nb_parallel * self._current_noise.shape[0]):
            coord_batch[2 * i, chosen_indices[i]] += self.variable_h
            coord_batch[2 * i + 1, chosen_indices[i]] -= self.variable_h
    ...
    def _optimizer_adam_coordinate(
        ...
        if proj and hasattr(self.estimator, "clip_values") and self.estimator.clip_values is not None:
            clip_min, clip_max = self.estimator.clip_values
            # ISSUE: need fix array lengths
            # current_noise[index] = np.clip(current_noise[index], clip_min, clip_max)
            current_noise[index] = np.clip(current_noise[index], clip_min[index], clip_max[index])
            # print(current_noise[index].shape)
    ...
    def _check_params(self) -> None:
        ...
        #if self.feature_indices is not None or not all(isinstance(i, int) for i in self.feature_indices):
        #    raise ValueError("The `feature_indices` should be a list of integers or None.")
